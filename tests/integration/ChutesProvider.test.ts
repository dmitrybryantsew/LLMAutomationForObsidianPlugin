/**
 * Integration Tests for ChutesProvider
 */
...
describe('ChutesProvider Integration', () => {
    let provider: ChutesProvider;
    let mockFetch: any;
    ...
    describe('generateText', () => {
        it('should generate text successfully', async () => {
            const mockResponse = {
                ...
                created: 1234567890,
                choices: [{
                    message: { content: 'Generated text from Chutes' }
                }]
            };
            ...
            expect(result.output).toBe('Generated text from Chutes');
            expect(result.metadata).toBeDefined();
            expect(result.metadata.provider_name).toBeDefined();
            expect(result.metadata.actual_model).toBe('chutes-model');
        });
        ...
        it('should use correct headers', async () => {
            ...
            expect(headers['HTTP-Referer']).toBeUndefined(); // Chutes doesn't use referer
        });
        ...
        it('should use correct API endpoint', async () => {
            ...
            const callArgs = mockFetch.mock.calls[0];
            expect(callArgs[0]).toBe('https://api.chutes.ai/v1/chat/completions');
        });
    ...
    describe('analyzeImage', () => {
        it('should analyze image successfully', async () => {
            const mockResponse = {
                id: 'test-id',
                model: 'chutes-vision-model',
                created: 1234567890,
                choices: [{
                    message: { content: 'Image analysis from Chutes' }
                }]
            };
            ...
            expect(result.analysis).toBe('Image analysis from Chutes');
            expect(result.metadata).toBeDefined();
        });
