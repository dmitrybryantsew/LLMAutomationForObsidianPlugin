...
describe('StudyHub and Command Catalog', () => {
  it('registers open-study-hub, open-quiz-hub, toggle-study-focus, toggle-coding-focus, and import-obsidian-spaced-repetition commands in catalog', () => {
    ...
    const importOsrCmd = PLUGIN_COMMAND_CATALOG.find((c) => c.id === 'import-obsidian-spaced-repetition');
    ...
    expect(importOsrCmd).toBeDefined();
    expect(importOsrCmd?.name).toBe('Import Cards from Spaced Repetition Plugin');
    expect(importOsrCmd?.group).toBe('Learning');
  });
  ...
  it('includes new study commands in rendered cheatsheet note', () => {
    ...
    expect(cheatsheet).toContain('`import-obsidian-spaced-repetition`');
    expect(cheatsheet).toContain('Open Study Hub');
    expect(cheatsheet).toContain('Open Quiz Hub');
    expect(cheatsheet).toContain('Toggle Study Hub Focus Mode');
    expect(cheatsheet).toContain('Toggle Coding Practice Focus Mode');
    expect(cheatsheet).toContain('Import Cards from Spaced Repetition Plugin');
  });
