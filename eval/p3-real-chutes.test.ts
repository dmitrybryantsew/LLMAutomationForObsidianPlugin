...
const { ChutesEmbeddingProvider } = await import('../src/retrieval/ChutesEmbeddingProvider');
const { DebugLogger } = await import('../src/utils/DebugLogger');
import type { RetrievalSettings, RetrievalSourceConfig, EmbeddingProvider } from '../src/types/retrieval';

const CHUTES_KEY = process.env.CHUTES_API_KEY || '';
const CHUTES_BASE = 'https://chutes-qwen-qwen3-embedding-8b-tee.chutes.ai';
const CHUTES_MODEL = 'Qwen/Qwen3-Embedding-8B-TEE';
...
function makeSettings(): RetrievalSettings {
  return {
    ...
    embedding: { provider: 'chutes', ollamaEndpoint: '', ollamaModel: '', chutesApiKey: CHUTES_KEY, chutesBaseUrl: CHUTES_BASE, chutesModel: CHUTES_MODEL, semanticThreshold: 0.3, lexicalVeto: true },
  };
}
...
describe('P3 real embedding eval (Chutes Qwen3-Embedding-8B)', () => {
  ...
  let provider: ChutesEmbeddingProvider;
  ...
  beforeAll(async () => {
    hasKey = CHUTES_KEY.length > 0;
    if (!hasKey) {
      console.log('No CHUTES_API_KEY env var — skipping Chutes eval');
      return;
    }
    ...
    debugLogger = new DebugLogger(false, 'eval-p3-chutes');
    ...
    provider = new ChutesEmbeddingProvider({ apiKey: CHUTES_KEY, baseUrl: CHUTES_BASE, model: CHUTES_MODEL, concurrency: 1 });
    ...
    console.log(`Embedding ${chunkDrafts.length} chunks with ${CHUTES_MODEL} (4096 dims)...`);
    ...
    writeFileSync(
      path.join(process.cwd(), 'eval', 'results', `p3-real-chutes-${new Date().toISOString().slice(0, 10)}.md`),
      report, 'utf8'
    );
  }, 600_000);
...
function formatComparisonReport(
  ...
  const lines: string[] = [];
  lines.push(`# P3 real embedding eval — Chutes Qwen3-Embedding-8B`);
  lines.push('');
  lines.push(`Generated: ${now}`);
  lines.push(`Fixture vault: ${FIXTURE_ROOT}`);
  lines.push(`Total queries: ${lexical.totalQueries} (per mode)`);
  lines.push(`Embedding provider: Chutes Qwen/Qwen3-Embedding-8B-TEE (4096 dims)`);
  ...
  lines.push(`- **Semantic-only**: Brute-force cosine similarity over real Chutes Qwen3-Embedding-8B (4096 dims)`);
